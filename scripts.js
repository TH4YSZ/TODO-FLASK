  ...
  dados.map((item) => {
    tabela.innerHTML += `
      <tr>
        ...
        <td>${item.TAREFA}</td>
        <td><span class="material-symbols-outlined text-danger" onclick="excluirTarefa(${item.ID})">
        delete
        </span><span class="material-symbols-outlined text-success" onclick="atualizarTarefa('${item.TAREFA}')">
        edit
        </span></td>
      </tr>
      `
